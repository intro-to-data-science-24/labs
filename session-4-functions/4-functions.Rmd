...
# Apply function and create emoticon column
study$emoticons <- replace_w_emoticons_vectorized(study$emotions)
...
geod_dist <- function(lat1, lon1, lat2, lon2, earth.radius = 6371) {
  ...
  lon1 <- deg2rad(lon1)
  lat1 <- deg2rad(lat1)
  lon2 <- deg2rad(long2) # replace with lon2
  lat2 <- deg2rad(lat2)
  # calculation
  delta.long <- (lon2 - lon1)
  delta.lat <- (lat2 - lat1)
  a <- sin(delta.lat/2)^2 + cos(lat1) * cos(lat2) * sing(delta.long/2)^2 # replace sing with sin
  c <- 2 * asin(min(1,sqrt(a)))
  d = earth_radius * c # replace earth_radius with earth.radius
  return(d)
}
...
# load packages
library(tidyverse)
library(LegislatoR) # legislatoR

# get political data on German legislators
political_df <- 
  left_join(x = filter(get_political(legislature = "ger"), as.numeric("session") == 18), # issue with the filter and retrieval, see solution
            y = get_core(legislature = "ger"), by = "pageid") # deu instead of ger

# wiki traffic data
traffic_df <- 
  get_traffic(legislature = "ger") %>% # deu instead of ger
...
# sessions served
sessions_served_df <- 
  get_political(legislature = "deu") %% # error in the pipe
  group_by(pageid) %>% 
...
get_age <- function(birth, date_at) {
  date_at_fmt <- date_at
  birth_fmt <- birth
  diff <- difftime(lubridate::ymd(birth_fmt), lubridate::ymd(date_at_fmt)) # switch death and birth
  diff_years <- time_length(diff, "years") # you need load the lubridate function here
  diff_years
}
...
legislator_df_table <- dplyr::select(rank, name, traffic_mean, traffic_max) # pass the dataframe
names(legislator_df_table) <- c("Rank", "Representative", "Mean", "Maximum") # change to colnames()
...
fmla <- paste("traffic_log", paste(covars, collapse = " - "), sep = " ~ ") # collapse with +
