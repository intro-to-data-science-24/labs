...
specs <- specr::setup(
  ...
  x = c("gdp", "log(gdp)", "I(gdp^2)", "gdp * total_expenditure"), # add independent variables 
...
# since we don't want to overfill the plot, we define a small list of models
models <- list(
...
We will cover data visualisation in R in much greater detail in the next session, so hopefully the section on **coefficient plot** will be made clearer then! 

***
# Exercises `r emo::ji("person_lifting_weights")`

If you want to practice the above you can find more materials [here](https://github.com/intro-to-data-science-23/labs/blob/main/practice-scripts/08-model-fitting.R). 
