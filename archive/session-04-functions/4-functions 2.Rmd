...
calc_mode <- function(m){
  freq_table <- table(m)
  my_mode <- names(freq_table[which.max(freq_table)])
  return(my_mode)
}

calc_mode2 <- function(m){
  freq_table <- table(m)
  my_mode <- names(which.max(freq_table))
  return(my_mode)
}
...
#replace_w_emoticons(study$emotions)
replace_w_emoticons_vector <- Vectorize(replace_w_emoticons)
study$emoticons <-study$emotions |> replace_w_emoticons_vector()
...
calc_mean_sd <- function(x){
  if(!(is.numeric(x))){
    stop("This is not a numeric value.")
  } else{
