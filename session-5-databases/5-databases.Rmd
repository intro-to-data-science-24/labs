...
We need to populate our database. We copy the data from earlier to our database connection. In **real life** this step would probably be taken care of by the responsible **database maintainer**.
...
The console output shows that this is a **remote source**; the table is not stored in our RStudio environment. Nor should you need to transfer the entire table to your RStudio environment. You can perform operations directly on the remote source. What is more you can rely on the `dplyr` syntax from earlier to formulate your queries. R will automatically translate it into SQL (more on that below).
