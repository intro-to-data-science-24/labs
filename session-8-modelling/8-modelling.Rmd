...
```
***
Welcome back! `r emo::ji("beach with umbrella")`
...
A quick recap on the syntax that you can use within formulas. The `~` sign generally differentiates between dependent and independent variables. The `+` sign is used to distinguish between independent variables. Here are a few other lines that are nice to know:   
...
formulas are very straightforward to work with. To avoid any errors you should follow the following two steps. 
...
As you can see the function in the lecture is quite powerful, but it is not the only option. Depending on the type of analysis that you plan to run certain packages have built-in options to run combinations of regression models in only a few lines (see for example the [multiple estimations vignette for the fixest package](https://cran.r-project.org/web/packages/fixest/vignettes/multiple_estimations.html)). 
...
***
# Exercises `r emo::ji("person_lifting_weights")`

If you want to practice the above you can find more materials [here](https://github.com/intro-to-data-science-22/labs/blob/main/practice-scripts/08-model-fitting.R). 
